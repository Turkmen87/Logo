...
- Çekler hareket gördükçe `LG_XXX_XX_CSTRANS` tablosunda çek referansı olarak (CSREF) de tutulur. Ve her hareket birden çok satıra dağılır.
...
- Çek hareketleri bordrolar aracılığı ile `LG_XXX_XX_CSTRANS` tablosuna yazılır. Devir çekleri hariç (Devir çekleri bordrolar aracılığı ile olmadığından) bordroların bilgilerinin yazıldığı `LG_XXX_XX_CSROLL` tablosunda sadece bordro başlık bilgileri bulunur.
- Bordro olarak işlediğiniz bütün bilgiler `LG_XXX_XX_CLFLINE` tablosuna ve çek kart bilgilerinizde vade kontrolleri için satır satır `LG_XXX_XX_PAYTRANS` tablosuna yazar. Oradan da Borç/Alacak toplam bilgileri oluşur.
...
**Çek Senet Hareketi (LG_XXX_XX_CSTRANS):**
...
Firma bağımsız tablolar, sistemdeki tüm firmalar için ortak kullanılan tablolardır. Bu tablolar genellikle "L_" öneki ile başlar.
...
## Tablo Yapısı Notları

1. Logo veritabanında tablo isimleri genellikle LG_XXX_YY_ZZZZ formatındadır:
   - LG: Logo öneki
   - XXX: Firma kodu (örn. 001, 002)
   - YY: Dönem kodu (örn. 01, 02)
   - ZZZZ: Tablo adı

2. Logo ERP sisteminde, firmaları ayırmak için genellikle firma numarasına (company number) göre ayrı tablolar veya şemalar kullanılır. 
   Logo'da tablolar genellikle aşağıdaki formatta adlandırılır:
   LG_{Firma No}_{Tablo Adı}

3. Firma bağımsız tablolar genellikle L_ öneki ile başlar.

4. Tablo isimlerindeki XXX yerine gerçek firma kodunuzu, XX yerine gerçek dönem kodunuzu yerleştirerek sorguları çalıştırmalısınız.

5. Birçok tabloda ortak alanlar bulunur:
   - LOGICALREF: Genellikle tablonun birincil anahtar alanıdır
